 ...
 * @author fonini
 */
@XmlRootElement(name = "weatherSource")
public class WeatherSourceConverter {
	 ...
	 */
	public WeatherSourceConverter(WeatherSource entity, URI uri, int expandLevel, boolean isUriExtendable) {
		this.entity = entity;
	 ...
	 */
	public WeatherSourceConverter(WeatherSource entity, URI uri, int expandLevel) {
	...
	/**
	 * Returns the WeatherSource entity.
	 *
	 * @return an entity
	 */
	@XmlTransient
	public WeatherSource getEntity() {
		if (entity.getWid() == null) {
			WeatherSourceConverter converter = UriResolver.getInstance().resolve(WeatherSourceConverter.class, uri);
			if (converter != null) {
	...
	/**
	 * Returns the resolved WeatherSource entity.
	 *
	 * @return an resolved entity
	 */
	public WeatherSource resolveEntity(EntityManager em) {
		Integer max = (Integer) em.createQuery("select Max(w.wid) from WeatherSource w").getSingleResult();
		if (max == null){
			max = 0;
		}
		entity.setWid(++max);
	...
	public boolean equals(Object object) {
		if (!(object instanceof WeatherSourceConverter)) {
			return false;
		}
		WeatherSourceConverter other = (WeatherSourceConverter) object;
