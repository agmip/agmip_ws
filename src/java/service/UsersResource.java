...
public class UsersResource {
    ...
    public UsersConverter get(@QueryParam("expandLevel")
            @DefaultValue("1") int expandLevel) {
        PersistenceService persistenceSvc = PersistenceService.getInstance();
        try {
            persistenceSvc.beginTx();
            return new UsersConverter(getEntity(), uriInfo.getAbsolutePath(), expandLevel);
        } finally {
     ...
     *
     * @param id identifier for the entity
     * @param data an UsersConverter entity that is deserialized from a XML stream
     */
    @PUT
    @Consumes({"application/xml", "application/json"})
    public void put(UsersConverter data) {
        PersistenceService persistenceSvc = PersistenceService.getInstance();
        try {
            persistenceSvc.beginTx();
    ...
    public String delete() {
        PersistenceService persistenceSvc = PersistenceService.getInstance();
        try {
			Integer id = getEntity().getUserId();
            persistenceSvc.beginTx();
    ...
    protected Users getEntity() {
        ...
        try {
            return (Users) em.createQuery("SELECT e FROM Users e where e.userId = :userId").setParameter("userId", id).getSingleResult();
     ...
     */
    private Users updateEntity(Users entity, Users newEntity) {
        EntityManager em = PersistenceService.getInstance().getEntityManager();
     ...
     */
    private void deleteEntity(Users entity) {
        EntityManager em = PersistenceService.getInstance().getEntityManager();
