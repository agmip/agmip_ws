...
public class MulchLevelsResource {
	...
	public Response post(MulchLevelConverter data) {
		...
		try {
			...
			persistenceSvc.commitTx();
			return Response.created(uriInfo.getAbsolutePath().resolve(entity.getMulchLevelPK().getExpId() + "," + entity.getMulchLevelPK().getMl() + "/")).build();
		}
		finally {
			persistenceSvc.close();
		}
	...
	protected void createEntity(MulchLevel entity) {
		EntityManager em = PersistenceService.getInstance().getEntityManager();
		em.persist(entity);
		for (MulchEvent value : entity.getMulchEventsCollection()) {
