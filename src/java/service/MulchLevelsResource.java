...
public class MulchLevelsResource {
    ...
    public MulchLevelsConverter get(@QueryParam("expandLevel")
            @DefaultValue("1") int expandLevel) {
        PersistenceService persistenceSvc = PersistenceService.getInstance();
        try {
            persistenceSvc.beginTx();
            return new MulchLevelsConverter(getEntity(), uriInfo.getAbsolutePath(), expandLevel);
        } finally {
    ...
    public void put(MulchLevelsConverter data) {
        PersistenceService persistenceSvc = PersistenceService.getInstance();
        try {
            persistenceSvc.beginTx();
    ...
    protected MulchLevels getEntity() {
        ...
        try {
            beans.MulchLevelsPK id = new beans.MulchLevelsPK(id1, id2);
            return (MulchLevels) em.createQuery("SELECT e FROM MulchLevels e where e.mulchLevelsPK = :mulchLevelsPK").setParameter("mulchLevelsPK", id).getSingleResult();
     ...
     */
    private MulchLevels updateEntity(MulchLevels entity, MulchLevels newEntity) {
        EntityManager em = PersistenceService.getInstance().getEntityManager();
     ...
     */
    private void deleteEntity(MulchLevels entity) {
        EntityManager em = PersistenceService.getInstance().getEntityManager();
    ...
    public static class TreatmentsCollectionResourceSub extends TreatmentssResource {
        ...
        private MulchLevels parent;

        public void setParent(MulchLevels parent) {
            this.parent = parent;
        }
